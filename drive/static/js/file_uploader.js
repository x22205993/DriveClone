class FileUploader {
    ...
    constructor(file, folder) {
        ...
        this.upload_queue_collapse_element = domHandler.getUploadQueueCollapseElemet()
    }
  
    async uploadFile() {
        this.addToFileQueueDisplay()
        const presigned_url_resp = await fileService.getPresignedUrlForUpload(this.file.name)
        let object_key = await presigned_url_resp.object_key
        await fileService.uploadFileUsingPresignedURL(presigned_url_resp.presigned_url, this.file)
        console.log(this.file.name, this.file_upload_id)
        console.log(this.folder)
        const create_file_resp = await fileService.createFile(object_key, this.file.name.trim(), this.folder.trim())
